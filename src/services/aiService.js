...
export const parseInvoice = async (input, isFile = false) => {
  ...
  const prompt = `
    ...
    - signature_name: Name of the person authorized to sign the invoice (usually at the bottom).
  ...
  try {
    let result;
    if (isFile) {
      // Input is expected to be a base64 string for PDF
      const part = {
        inlineData: {
