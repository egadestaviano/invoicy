      ...
      <div v-if="isOpen" class="relative w-screen max-w-md bg-white pointer-events-auto flex flex-col h-full border-l border-slate-200">
        ...
        <div class="p-6 border-b border-slate-200 flex items-center justify-between bg-white">
          <div class="flex items-center gap-3">
            <div class="h-10 w-10 border border-orange-200 bg-orange-500 flex items-center justify-center">
            ...
            <div>
              <h2 class="text-xl font-bold text-slate-900 leading-tight">AI Invoice Parser</h2>
              <p class="text-xs font-medium text-slate-500 uppercase tracking-wider">Automated Extraction</p>
            </div>
          </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
          <div class="space-y-4">
            <label class="block">
              <span class="text-sm font-bold text-slate-700 flex items-center gap-2 mb-2">
              ...
              <div 
                class="relative group cursor-pointer border-2 border-dashed rounded-sm p-8 transition-all duration-300"
                :class="[
                  pdfFile ? 'border-slate-800 bg-slate-50' : 'border-slate-200 hover:border-slate-400 hover:bg-slate-50'
                ]"
                @dragover.prevent
              ...
              >
                ...
                <div class="text-center">
                  ...
                  <div v-else class="flex items-center justify-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-orange-500"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                    <div class="text-left">
                      <p class="text-sm font-bold text-slate-900 truncate max-w-[200px]">{{ pdfFile.name }}</p>
                      <button @click.stop="clearFile" class="text-xs font-semibold text-red-600 hover:text-red-700">Remove file</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="mt-2 flex items-center justify-between px-1">
              <span class="text-[10px] font-medium text-slate-400">Need a test file?</span>
              <a 
                href="/sample-invoice.pdf" 
                download="sample-invoice.pdf"
                class="text-[10px] font-bold text-orange-500 hover:text-orange-600 flex items-center gap-1 transition-colors"
              >
            ...
            <div class="relative py-4 flex items-center gap-4">
              ...
              <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">OR PASTE TEXT</span>
              <div class="flex-1 h-px bg-slate-100"></div>
            </div>

            <label class="block">
              <span class="text-sm font-bold text-slate-700 flex items-center gap-2 mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-orange-500"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                Paste Invoice Text
              </span>
              <textarea 
                v-model="rawText" 
                rows="8" 
                class="w-full rounded-sm border-slate-200 focus:border-slate-800 focus:ring-slate-800 text-sm p-4 transition-all"
                placeholder="Paste the raw text from an invoice here..."
              ></textarea>
              
              <!-- Example Text Copy -->
              <div class="mt-2 flex items-center justify-between px-1">
                <span class="text-[10px] font-medium text-slate-400">No invoice text?</span>
                <button 
                  @click="useExampleText"
                  class="text-[10px] font-bold text-orange-500 hover:text-orange-600 flex items-center gap-1 transition-colors cursor-pointer"
                >
        ...
        <!-- Footer Actions -->
        <div class="p-6 border-t border-slate-200 space-y-3 bg-white">
          <button 
            @click="handleParse" 
            :disabled="isParsing || (!rawText && !pdfFile)"
            class="w-full flex items-center justify-center gap-3 px-6 py-4 bg-orange-500 hover:bg-orange-600 disabled:bg-slate-300 text-white rounded-sm font-bold transition-all active:scale-[0.98] cursor-pointer"
          >
...
Bank: Central Bank, Acc: 1234567890 (Ega Destaviano)
